...
export const getDelayPredictionAPI = async (
  origin,
  destination,
  depDate,
  depTime,
...
export const getMultiHotelsOffers = async (
  hotelIDs,
  checkInDate,
  checkOutDate,
  adults = 1,
  ...
  console.log(process.env.AMADEUS_API_KEY)
  // encode the array of hotel id strings for the url get request
  const arr_str = encodeURIComponent(JSON.stringify(hotelIDs))
  try {
    const response = await fetch(
      `https://test.api.amadeus.com/v3/shopping/hotel-offers?hotelIds=${arr_str}&adults=${adults}&checkInDate=${checkInDate}&checkOutDate=${checkOutDate}&roomQuantity=${roomQuantity}&bestRateOnly=${bestRateOnly}`,
      {
        headers: {
          Authorization: `Bearer ${API_KEY}`
        }
...
export const getHotelNameAutocomplete = async (
  keyword,
  countryCode = 'US',
  lang = 'ENG',
  max = 15
